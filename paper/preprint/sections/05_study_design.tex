...
We evaluated MLReproMutate on a frozen corpus of 39 repository--operator cases
...
\texttt{dependency-pin} changes an exact dependency constraint from
\texttt{package==version} to \texttt{package>=version}.
...
Semantic verification distinguished
\texttt{confirmed-non-equivalent},
\texttt{confirmed-equivalent},
\texttt{unverified}, and
\texttt{not-run} states. Confirmed equivalent mutations were excluded from the
meaningful detection denominator.
